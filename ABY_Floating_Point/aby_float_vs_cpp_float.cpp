...
double test_verilog_float_SIMD(e_role role, const std::string& address, uint16_t port, seclvl seclvl, uint32_t nvals, uint32_t nthreads,
	e_mt_gen_alg mt_alg, e_sharing sharing, double afp, double bfp) {
	...
	std::fill(avals, avals + nvals, *aptr);
	std::fill(bvals, bvals + nvals, *bptr);
	std::fill(input_values, input_values+1, *aptr);
	...
	share* ain = circ->PutSIMDINGate(nvals, avals, bitlen, SERVER);
	share* bin = circ->PutSIMDINGate(nvals, bvals, bitlen, CLIENT);
	share* x_val_in_s = circ->PutSIMDINGate(1, input_values, bitlen, SERVER); // This holds server's x input
	share* x_val_in_c = circ->PutSIMDINGate(1, input_values, bitlen, CLIENT); // This holds client's x input
	...
	// FP multiplication gate with ieee_compliance flag turned on.
	share* mul = circ->PutFPGate(x_val_in_s, x_val_in_c, MUL, bitlen, 1, no_status);
	//for(int i=0; i<18; i++){
		//mul = circ->PutFPGate(x_val_in_s, mul, MUL, bitlen, 1, no_status);
	//}
...
int main(int argc, char** argv) {
	...
	for(int i=0; i<125; i++){
		...
		double fpa = get_random_double();
		double fpb = get_random_double();
		if(role == CLIENT)
			res_aby = test_verilog_float_SIMD(role, address, port, seclvl, nvals, nthreads, mt_alg, S_BOOL, fpa, 0.0);
		else
			res_aby = test_verilog_float_SIMD(role, address, port, seclvl, nvals, nthreads, mt_alg, S_BOOL, fpb, 0.0);

		res_cpp = fpa*fpb;
		if(res_aby != res_cpp){
			cout<<"Mult results don't match between ABY and CPP on iter "<<i<<endl;
			cout<<"Inputs were: "<<fpa<<" and "<<fpb<<endl;
			cout<<"ABY gives: "<<res_aby<<endl;
			...
			//cout<<"Mult results MATCH between ABY and CPP on iter "<<i<<endl;
			//cout<<"Inputs were: "<<fpa<<" and "<<fpb<<endl;
			//cout<<"ABY gives: "<<res_aby<<endl;
			//print_bits_long_type(&res_aby, true);
			//cout<<"CPP gives: "<<res_cpp<<endl;;
